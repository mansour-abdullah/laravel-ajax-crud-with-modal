                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody id='tbody'>
                                @foreach ($cars as $car)
                                <tr id='{{ $car->id }}'>
                         <td id='carBrand{{ $car->id }}'>{{ $car->Brand }}</td>
                          <td id='carModel{{ $car->id }}'>{{ $car->model }}</td>
                           <td id='carYear{{ $car->id }}'>{{ $car->production_year }}</td>
                             <td><button data-toggle="modal" data-target="#myModal" class="btn btn-xs btn-primary open-modal" value="{{$car->id}}"><i class="glyphicon glyphicon-edit"></i> Edit</button></td>
                             <td><button class="btn btn-xs btn-danger delete" value="{{$car->id}}"><i class="glyphicon glyphicon-trash"></i>Delete</button></td>
                                </tr>
                                     @endforeach
                            </tbody>
                        </table>
  ...
  <div class="modal-dialog">
   ...
   <script>
        var token = '{{ Session::token() }}';
         var url = "{{ route('home') }}";
       var urlEdit = "{{ route('car.update') }}";
         var urlAdd = "{{ route('car.add') }}";
